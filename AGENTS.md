...
Classroom ERP. Language lives in `CONTEXT.md`. Do not reopen what is already decided.
...
## UI — Playwright, do not persist

At the **end** of every implementation that touches UI or a journey, walk **every** journey in the work with Playwright (MCP or a throwaway spec). Empty, create, duplicate, delete, and whatever the slice lists.

A green `bun test` on the API **does not** replace this.

Do not commit Playwright specs, an `e2e/` folder, screenshots, or fixtures. If you created a file to run it, delete it before you finish.
